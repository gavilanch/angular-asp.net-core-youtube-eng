...
namespace MyWebAPI.Controllers
{
    ...
    public class LaptopsController : ControllerBase
    {
        ...
        [HttpGet("{name}/exists")]
        public async Task<ActionResult<bool>> ExistsLaptopWithName(string name, int id)
        {
            await Task.Delay(3000);
            if (id == 0)
            {
                return await context.Laptops.AnyAsync(x => x.Name == name);
            } else
            {
                return await context.Laptops.AnyAsync(x => x.Name == name && x.Id != id);
            }
        }
